...
class DataValidator:
    ...
    def validate_data(self, data_path):
        ...
        try:
            ...
            self.validation_report['missing_values'] = missing_values[missing_values > 0].to_dict()
            self.validation_report['missing_percentage'] = missing_percentage[missing_percentage > 0].to_dict()
            
            if missing_values.sum() > 0:
                logger.warning(f"Обнаружены пропущенные значения: {missing_values.sum()}")
